...
- `a|v|s`：指定输入文件所指的流类型。a 表示音频（audio），v 表示视频（video），s 表示字幕（subtitle）。
- `n`：是指的具体流，从0开始。

听起来感觉很复杂，其实不然，举几个例子就理解了：

$1.$ `0:a`：指引用第一个文件的所有音频流（audio）<br/>
$2.$ `0:v`：指引用第一个文件的所有视频流（video）<br/>
$3.$ `0:a:1`：指引用第一个文件的第二个音频流<br/>
$4.$ `0:v:0`：指引用第一个文件的第一个视频流<br/>
$5.$ `1:v:0`：指引用第二个文件的第一个音频流<br/>
