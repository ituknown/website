...
映射索引为 0 的输入文件（input.mp4）的所有视频流。一个媒体文件通常只有一个视频流，即主视频。

如果该索引文件有多个视频流，可以通过索引指定。比如该文件有两个视频流，就可以使用下面的方式指定要映射视频流的索引（只需要映射以及需要的流即可）：
...
与 `-map 0:v` 表示同样的含义，只不过这个映射的是音频流。
</details>

<details open>
<summary>**`-map 1:v`**</summary>

将索引为1 $1$ 的输入文件（cover.jpg）映射为视频流。

视频其实就是图片序列，所以将图片映射为视频流没有任何问题。
</details>

<details open>
<summary>**`-c copy`**</summary>

对映射（`map`）的流只做复制操作，不重新编码。就是说保留原视频流、音频流的编码格式，不做任何操作（最主要的是对流重新编码特别浪费时间）。
</details>
...
该命令中一共映射了两个视频流，具体使用哪一个呢？所以就需要继续追加索引 $1$ 来指定（对应的就是输入文件 cover.jpg）。
...
这个元数据非必须，但是如果你修改的是 flac 音频文件，那该元数据就是必须的，且值也必须指定为 `Cover (front)`。
</details>
...
将第二个视频流标记为封面（封面图片），这个必须要设置。因为很多主流的播放器都是以 attached_pic 来判断是否设置了封面。
</details>
